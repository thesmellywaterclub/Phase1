...
export default function ShoppingCartPage() {
  ...
  const shipping =
    shippingRate ??
    (taxableSubtotal >= 199 ? 0 : taxableSubtotal > 0 ? 9.99 : 0);
  ...
  const calcShipping = useCallback(() => {
    ...
    const near = isValidZip.test(zip) ? Number.parseInt(zip[0], 10) < 6 : false;
    setShippingRate(near ? 6.99 : 12.99);
  ...
  function updateQty(product: Product, variant: ProductVariant, next: number) {
    const clamped = Math.max(
      1,
      Math.min(next, variant.stock ?? Number.POSITIVE_INFINITY)
    );
              ...
              {isEmpty ? (
                ...
                <div className="space-y-4">
                  {detailedItems.map(({ key, product, variant, qty }) => {
                    const stockLimit =
                      variant.stock ?? Number.POSITIVE_INFINITY;
                    const limitReached =
                      Number.isFinite(stockLimit) && qty >= stockLimit;
                    return (
